...
const LINK_NODE_URL = window.HOST + '/api/v1/linkrepair/links'
const LINK_NODE_REPAIR_URL = window.HOST + '/api/v1/linkrepair/repair'
  ...
  methods: {
    ...
    fetchData (oParam) {
      ...
      this.$register.sendRequest(
        ...
        LINK_NODE_URL,
        'post',
        fnP.parseQueryParam(oParam.keys),
        oData => {
          that.loading = false
          // 数据保存。
          this.$store.commit({
    ...
    getHeight () {
      ...
      let vFilter = this.$refs.containerFilter
      let vInfo = this.$refs.mainInfo
    ...
    getNodeIndexById (sId) {
      ...
      // let aNode = this.pageData.doInOuts.filter(function (oNode, n) {
      //   if (oNode.id === sId) {
      //     nIndex = n
      //     return true
      //   } else {

...
var bFull = window.location.hash.indexOf('full') > -1
...
export default {
  ...
  data () {
    ...
    return {
      ...
      bSubmit: !!bFull,
      // 是否大屏轮播。
      bCarousel: window.location.hash.indexOf('carousel') > -1,
      // 是否显示标题。  只有当调用成功后才显示。
      bShowTitle: false,
      // 查询搜索值。
      ruleForm: {
      ...
      // 类型。
      oTab: {
        url: window.HOST + '/api/v1/resume/search',
        columns: [
          {
            prop: 'materialName',
            name: '',
  ...
  methods: {
      ...
      if (node.scrollHeight > node.clientHeight) {
        setInterval(function () {
          if (bEnd) {
            ...
            node.scrollTop + 100 + node.clientHeight >
            node.scrollHeight
          ) {
    ...
    parseTimeLineData (aoData) {
      // 按照日期排序。
      let aoNewData = aoData.sort(
        (oA, oB) => (+new Date(oA.date) - +new Date(oB.date) < 0 ? 1 : -1)
      )

      // 按时间排序。
      aoNewData.forEach(
        o =>
          (o.resumes = o.resumes.sort(
            (oA, oB) =>
              +new Date(oA.opTime) - +new Date(oB.opTime) < 0 ? 1 : -1
          ))
      )
      ...
      if (aData.length > 1) {
        ...
        aOrder.forEach((o, index) => {
          if (index !== aOrder.length - 1 && o.data.length > 1) {
            ...
            o.data = o.data.sort(
              (oA, oB) =>
                +new Date(oA.data.time) - +new Date(oB.data.time) < 0 ? 1 : -1
            )
          }
    ...
    getChildrenIndexArrByParentIndex (nIndex) {
      let aIndex = []
        // 格式转换后的数据。
      let aoTableData = this.aParsedData
      let bFounded = false

      if (nIndex !== -1) {
        let oNow = aoTableData[nIndex]
        let nLevel = oNow.level
    ...
    expandOrCollapseTr (index, props) {
     ...
     // let nLevel = props.row.level
      ...
      const trs = document
        ...
        .querySelectorAll('tr')
      const trsFix = document
        .querySelectorAll('.el-table__fixed-body-wrapper')[0]
        .querySelectorAll('tr')
      for (let i = 0; i < num; i++) {
    ...
    // 下载功能。
    onDownLoadHandler () {
     // let jDown = $('#downloadImage')
      let jTimeLine = $('.time-line-content')
      let jClone = $('.clone')
      let sFileName = ''
      let jNow = null
      let isTable = false

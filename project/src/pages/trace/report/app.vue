...
//			 this.fetchData();
		...
		mounted() {
			let oConditions = sessionStorage.getItem("fastReport_" + this.params.tag);
			
			if(oConditions) {
				oConditions = JSON.parse(oConditions);
				
				this.setFilters(oConditions.filters, oConditions.length, oConditions.selected.length);
				this.setData(oConditions.selected);
			}
		},
		methods: {
			...
			// 设置筛选条件。
			setFilters(oFilters, wholeLenghth, selectedLength) {

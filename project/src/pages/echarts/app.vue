...
// }
        ...
        if (bExpand) {
          // 删除属性
          // 获取当前产出对应的投入数据。
          let aIn = this.aoAllData.filter(
            o => o.outputOpIdList && o.outputOpIdList.includes(sOpId)
          )
    ...
    createCell (params) {
      ...
      return `<div class="cell-content" style="padding-left: ${nPLeft}px">
           ${
              params.data.hasIn
                ? `<i class="icon-down el-icon-arrow-down ${
                    params.data.isExpand ? 'actived' : ''
                  }"></i>`
                : ''
            }
           <span>haha</span>
    ...
    selectionChanged (event) {
      ...
      if (aSelected.length) {
        this.gridOptions.api.getRenderedNodes().forEach(o => {
          // 数据存在selected中。
          if (
            that.selected.some(el => el.index === o.data.index) &&
            !o.selected
          ) {
            o.setSelected(true)
          }

<template>
	<div id="app">
		...
		<div class="main">
			<WipPriceTable :table-data="totalData" 
...
<script>
	...
	const url = HOST + '/api/v1/wip/price';
	
    export default {
    	components: {
    		'WipPriceTable': WipPriceTable,
    		'WipTitle': WipTitle
    	},
    	data() {
    		return {
    		...
    		this.$ajax.get(url).then(function (res) {
    			...
    			if (arr.length) {
    				arr.forEach(function (item) {
    					item.flag = true;//true:来自后台的数据; false:页面新添加的数据
    				});
    			}

        ...
        <div class="report-item report-question-describe">
          ...
          <ul class="item-content">
            <li class="descripe-item" v-for="(question,key) in pageData.describe" :key="key">
              <el-row>
                <el-col :span="4">
                  <span class="descripe-item-title">{{ getDescripeDisplay(key).title }}</span>
                </el-col>
                <el-col :span="4">
                  <span class="descripe-item-name">{{ getDescripeDisplay(key).name }}</span>
                </el-col>
                <el-col :span="16">
...
export default {
  ...
  computed: {
    ...
    information () {
      ...
      let informationArr = Object.assign({}, this.filters) || {}
      let needArr = []
      arr.forEach(element => {
        if (
  ...
  created () {
    ...
    let oConditions = sessionStorage.getItem(
      'restrainReports_' + this.query.tag
    )
  ...
  methods: {
    ...
    setHeight () {
      ...
      this.contentHeight =
        body - this.outerHeight(header) - this.outerHeight(footer) - 140
    },
    // 获取数据。
    fetchData () {
    ...
    setPageEditedState () {
      ...
      // 问题分析中 ，过滤没有图片及提示的代码。
      this.pageData.analysis = this.pageData.analysis.filter(
        o => o.imgs.length || o.content
      )
      // 保存数据
    ...
    beforeRemoveImage (file, fileList, $event) {
      ...
      anImg = anImg.filter(o => Number(o.uid) !== Number(file.uid))
    ...
    addNewAnalysisItem () {
      ...
      this.pageData.analysis.push({
        imgs: [],
        content: '',
        index: this.pageData.analysis.length
          ? this.pageData.analysis[this.pageData.analysis.length - 1].index + 1
          : 0
      })
    },
    // 删除
    delAnalysisItem (scope) {
      // 更新当前行。
      this.pageData.analysis = this.pageData.analysis.filter(
        (o, index) => !this.analysisSelectedIndex.includes(index)
      )
    },
      ...
      let sHtml = `
        <div class="report-wrapper">
            <div class="report-container">
                <div class="report-header">${
                  oPage.querySelector('.report-header').innerHTML
                }</div>
    ...
    downOrPrintPage (refHtml) {
      ...
      if (this.isDownload) {
        window.Rt.utils.rasterizeImgHTML(
          rasterizeHTML,
          sHtml,
          {

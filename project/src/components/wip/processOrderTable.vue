<template>
  	<el-table tooltip-effect="dark"
		:data="tableData" border 
		style="width:100%;height:100%;" max-height="400px;">
		
	    <el-table-column label="地点">
	    	<template scope="scope">
	    		<el-input v-model="scope.row.location"
	    			@change="changeHandler(scope.row)"
	    			@blur="inputBlurHandler(scope.row)"></el-input>
    		</template>
	    </el-table-column>
	    <el-table-column label="工序">
	    	<template scope="scope">
	    		<el-input v-model="scope.row.process"
	    			@change="changeHandler(scope.row)"
	    			@blur="inputBlurHandler(scope.row)"></el-input>
    		</template>
	    </el-table-column>
	    <el-table-column label="工序编码">
	    	<template scope="scope">
	    		<el-input v-model="scope.row.processCode"
	    			@change="changeHandler(scope.row)"
	    			@blur="inputBlurHandler(scope.row)"></el-input>
    		</template>
	    </el-table-column>
	    <el-table-column label="工序顺序">
	    	<template scope="scope">
	    		<el-input v-model="scope.row.processOrder"
	    			@change="changeHandler(scope.row)"
	    			@blur="inputBlurHandler(scope.row)"></el-input>
    		</template>
	    </el-table-column>
	    <el-table-column label="投产类型" prop="putType">
	    	<template scope="scope">
	    		<el-select v-model="scope.row.putType" placeholder="请选择" @change="changeHandler(scope.row, scope.column)">
				    <el-option
				      	v-for="item in options"
				      	:key="item.value"
				      	:label="item.label"
				      	:value="item.value">
				    </el-option>
			  	</el-select>
    		</template>
	    </el-table-column>
	    <el-table-column label="投产顺序">
	    	<template scope="scope">
	    		<el-input v-model="scope.row.putOrder"
	    			@change="changeHandler(scope.row)"
	    			@blur="inputBlurHandler(scope.row)"></el-input>
    		</template>
	    </el-table-column>
	    <el-table-column width="90" label="操作">
	      	<template scope="scope">
	      		<el-button type="primary" icon="delete" @click="deleteClick(scope.row)"></el-button>
	      	</template>
	    </el-table-column>
    
  	</el-table>
</template>

<script>
	export default {
		data: function () {
			return {
				options: [
					{'label': '投入', value: '投入'},
					{'label': '产出', value: '产出'}
				],
				filterFields: ['id', 'flag', 'changed']
			}
		},
		props: {
			tableData: {
				type: Array,
				default: []
			}
		},
		methods: {
			inputBlurHandler: function (row) {//输入框失去焦点或下拉选项改变
				let flag = true;//数据完整
				for (const key in row) {
					if (this.filterFields.indexOf(key) === -1 && !row[key]) {
						flag = false;//有空列
						break;
					}
				}
				
				if (flag && row.changed) {
					//更新到数据库
					this.$emit('save', row);
					row.changed = false;
				}
			},
			changeHandler: function (row, column) {
				row.changed = true;
				if (column && row[column.property]) {
					this.inputBlurHandler(row);
				}
			},
			deleteClick: function (row) {
				this.$emit('delete-by-id', row);
			}
		}
	}
</script>
...
const MODULE_ADD_DATA_URL =
  window.HOST + '/api/v1/customized/equipment-analysis/items'
// 模块修改。
...
const MODULE_DELETE_DATA_URL =
  window.HOST + '/api/v1/customized/equipment-analysis/items/'
// 设备数据。
const EQUIPMENT_DATA_URL = window.HOST + '/api/v1/basicinfo/equipmentIds'
...
export default {
  ...
  created () {
    ...
    this.$register.sendRequest(
      ...
      null,
      oData => {
        this.aoEquipment = JSON.parse(
          JSON.stringify(oData.equipmentIdInfoList)
        )
      },
      sErrorMessage => {
  ...
  methods: {
            ...
            for (let p in this.oBefore) {
              if (this.oBefore[p] instanceof Array) {
                if (this.oBefore[p].length !== this.oCurrentData[p].length) {
                  ...
                  if (
                    this.oBefore[p].some(
                      (value, index) => this.oCurrentData[p][index] !== value
                    )
                  ) {
                    bEqual = false
                    break
    ...
    saveModule () {
      this.$register.sendRequest(
        this.$store,
        this.$ajax,
        MODULE_EDIT_DATA_URL + this.oCurrentData.id,
        'put',
        this.oCurrentData,
        oData => {
    ...
    deleteModule () {
      this.$register.sendRequest(
        this.$store,
        this.$ajax,
        MODULE_DELETE_DATA_URL + this.oCurrentData.id,
        'delete',
        null,
        oData => {

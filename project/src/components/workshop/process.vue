<template>
    <div class="router-content">
        <div class="innner-content" :style="styleObject">
            <div class="condition" ref='condition' v-if="false">
                        ...
                        <el-table class="table-main" :data="checked?uniteItems.data:uniteItems.dataFilter" :height="uniteItems.height" stripe border style="width: 100%;" v-loading="loading" element-loading-text="拼命加载中" row-class-name="table-item">
                            <el-table-column v-for="(column,index) in uniteItems.columns" :key="index" :align="'center'" :fixed="index===0?true:false" :resizable="true" :label="column.name" :width="column.width">
                                <template scope="props">
                                    ...
                                    <!--<div class="cell-content v-else-if="column.prop =='equipmentName'">
                                    	<span class="equipment" @click="equipmentClick(props.row)">{{ column.formatter?column.formatter(props.row):props.row[column.prop] }}</span>
                                    </div>-->
                                    <div class="cell-content" v-else>
                                        <span v-if="column.prop =='equipmentName'" class="equipment" @click="equipmentClick(props.row)">{{ column.formatter?column.formatter(props.row):props.row[column.prop] }}</span>
...
//					o.hasIn = false
			...
			// 获取投产、有投无产、产出、投入的根据时间过滤后的数据-- 当前不需要过滤

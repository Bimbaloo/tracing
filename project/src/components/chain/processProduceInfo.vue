  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        happenTime: [{ validator: validateTime, trigger: 'change' }]
      }
    }
  },
  computed: {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.aList.forEach(function (oParam) {
            let sKey = oParam.key
            let sParam =
                'out' + sKey.substring(0, 1).toUpperCase() + sKey.substring(1) // sKey.charAt(0).toUpperCase() + sKey.slice(1)

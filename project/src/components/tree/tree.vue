...
// this.tree.commitTransaction("add nodes")
      ...
      return this.$(
        ...
        this.$(
          ...
          'Auto',
          {
            ...
            padding: new window.go.Margin(0, 20, 0, 0) // b,l,t,r
          },
          this.$(
            window.go.Shape,
            {
            ...
            // new window.go.Binding("fill", "isHighlighted", h => h ? HIGHLIGHT_BG_COLOR : "#ffffff")
          ),
          this.$(
            window.go.Picture,
            {
    ...
    workshopTemplate () {
      return this.$(
        ...
        this.$(
          ...
          this.$(
            window.go.TextBlock,
            new window.go.Binding('text', 'destAdjustQuantity'), // 源条码
            new window.go.Binding('width', 'column1'),
            {
              column: 1,
              margin: 5,
          ...
          this.$(
            ...
            new window.go.Binding('itemArray', 'list'),
            {
              itemTemplate: this.$(
                ...
                this.$(
                  window.go.TextBlock,
                  // new window.go.Binding("text", "", o => o.remainQuantity + "/" + o.destAdjustQuantity), // 调整数/滞留数
                  new window.go.Binding('text', '', function (o) {
                    if (o.remainQuantity === undefined) {
                      // 溯源处理 | 老版本处理
  ...
  methods: {
    ...
    treeExpanderButtonClickHandle (e, button) {
      ...
      if (node instanceof window.go.Adornment) {
    ...
    setCommentTemplate () {
      // 物料节点注释样式。
      this.tree.nodeTemplateMap.add(
        'materialComment',
        // this.$(window.go.Node,  // this needs to act as a rectangular shape for BalloonLink,
        ...
        this.$(
          ...
          this.$(window.go.Shape, {
            fill: COMMENT_BGCOLOR,
            stroke: 'lightgray',
            strokeWidth: 1
          }),
          ...
          this.$(
            ...
            new window.go.Binding('itemArray', 'items'),
            {
              ...
              itemTemplate: this.$(
                ...
                this.$(
                  window.go.TextBlock,
                  new window.go.Binding('text', 'equipment'), // 设备
                  { column: 0, margin: 5 }
                ),
                this.$(
                  window.go.TextBlock,
            ...
            this.$(
              ...
              this.$(
                window.go.TextBlock,
                new window.go.Binding('text', 'material'), // 物料名称
                {
                  row: 0,
                  column: 0,
                  columnSpan: 3,
      ...
      this.tree.linkTemplateMap.add(
        'Comment',
        // if the BalloonLink class has been loaded from the Extensions directory, use it
        // this.$((typeof BalloonLink === "function" ? BalloonLink : window.go.Link),
        // this.$(window.go.Shape,  // the Shape.geometry will be computed to surround the comment node and
        ...
        this.$(
          ...
          this.$(window.go.Shape, { stroke: COMMENT_BGCOLOR }),
          this.$(window.go.Shape, {
            stroke: COMMENT_BGCOLOR,
            toArrow: 'Standard',
            fill: COMMENT_BGCOLOR
          })
        )
        // this.$(window.go.Link,
        // { curve: window.go.Link.Bezier},
        // this.$(window.go.Shape, { stroke: COMMENT_BGCOLOR, strokeWidth: 2 }),
        // this.$(window.go.Shape, { stroke: COMMENT_BGCOLOR, toArrow: "Standard", fill: COMMENT_BGCOLOR })
        // )
      )
    },
    // 画布双击事件。
    ...
    // 画布初始化。
    initDiagram () {
      this.tree = this.$(window.go.Diagram, this.treeId, {
    ...
    drawTree () {
      ...
      this.tree.addDiagramListener(
        'LayoutCompleted',
        this.layoutCompletedHandle
      )
    },
    // 布局完成后的事件处理，设置初始时节点在中间显示。
    ...
    fitToCurrentKey () {
      if (this.treeData.node) {
        ...
        let oData = this.treeData.node.filter(
          o => (this.key ? o.key === this.key : o.parents.includes('0'))
        )[0]
     ...
     */
    createOverview () {
      this.overview = this.$(window.go.Overview, 'overview', {
        observed: this.tree,
    ...
    setSelection () {
      ...
      let oNode = this.tree.findNodeForKey(oData.key)

...
//      	}
        ...
        methods: {
            handleChange(value) {
            	...
            	if(value.length) {
	            	if((value.includes(this.allData) && value.length > 1) || (!value.includes(this.allData) && value.length === aGet.length )) {
	            		// 含有全部，则将数据设置为全部。
            		...
            		if(sQuery !== "") {
            			...
            			setTimeout(()=> {
            				...
            				this.options = this.list.filter(o=>{
            					...
            					if(sLable != this.allDataLabel && (sLable.indexOf(sQuery)>-1||o.label.indexOf(sQuery)>-1 || o.value.indexOf(sQuery)>-1) && nFilter<nLen){
            						nFilter++;
            						return true;
            					}else {
            						return false;

...
export default {
  ...
  data () {
    return {
            ...
            poolInList: [
              {
                ...
                doCode: 'D201603310017',
                materialName:
                  'ZC/SGE LFV 活塞总成/环销卡簧连杆/新型线/12667058',
            ...
            submitQcList: [
              {
                ...
                checkPersonName: '李瑞娇',
                reportpath: '\\192.168.1.2\reportXXXXYYYY'
              }
            ]
          }
        ]
      ...
      loading: false,
      sErrorMessage: '',
      url: window.HOST + '/api/v1/trace/equipments-events',
      // 比例。
      ratio: 1,
      // 设备状态。
      states: [
    ...
    checkedEquipments: function () {
      this.equipments.forEach(
        o =>
          ...
          (this.equipmentData[o.equipmentId].selected =
            this.checkedEquipments.indexOf(o.equipmentId) > -1)
      )
    },
    // 为了每次点击都会查询。
    $route: function () {
  ...
  methods: {
    ...
    formatEquipmentData (sType, aoData) {
      ...
      switch (sType) {
        case '1':
          aoData.forEach(
            oData =>
              this.equipmentData[oData.equipmentId] &&
              (this.equipmentData[oData.equipmentId].status =
                oData.equipStatusList)
          )
          break
        case '2':
          aoData.forEach(
            oData =>
              this.equipmentData[oData.equipmentId] &&
              (this.equipmentData[oData.equipmentId].work = oData)
          )
          break
        case '3':
          aoData.forEach(
            oData =>
              this.equipmentData[oData.equipmentId] &&
              (this.equipmentData[oData.equipmentId].quality = oData)
          )
          break
        case '4':
          aoData.forEach(
            oData =>
              this.equipmentData[oData.equipmentId] &&
              (this.equipmentData[oData.equipmentId].event = oData)
          )
          break
        case '5':
          aoData.forEach(
            oData =>
              this.equipmentData[oData.equipmentId] &&
              (this.equipmentData[oData.equipmentId].repair = oData)
          )
          break
        case '6':
          aoData.forEach(
            oData =>
              this.equipmentData[oData.equipmentId] &&
              (this.equipmentData[oData.equipmentId].tool = oData)
          )
          break
        default:
      }
    ...
    setDateWindowTime () {
      ...
      if (nInterval > 30) {
        ...
        this.windowTime.end = new Date(
          new Date(this.windowTime.start).getTime() +
            this.windowTime.interval * 60 * 1000
    ...
    moveSlider (event) {
      // 设置滑块位置。
      let maxLeft =
          event.currentTarget.offsetWidth - this.$refs.slider.offsetWidth
      let nRatio = 0
      ...
      if (event.target.getAttribute('class').indexOf('icon-start') > -1) {
        ...
        nRatio = this.formatData(
          maxLeft * 100 / event.currentTarget.offsetWidth,
          2
        ) // Math.floor
        ...
        nRatio = this.formatData(
          left * 100 / event.currentTarget.offsetWidth,
          2
        ) // Math.floor
      }

      let nStart =
        new Date(this.datetime.start).getTime() * (1 - nRatio / 100) +
        nRatio / 100 * new Date(this.datetime.end).getTime()
      this.windowTime.start = new Date(nStart).Format('yyyy-MM-dd hh:mm:ss')
      this.windowTime.end = new Date(
        nStart + this.windowTime.interval * 60 * 1000
      ).Format('yyyy-MM-dd hh:mm:ss')
    ...
    dragSlider (event) {
      ...
      let nLeft = parseFloat(slider.style.left.split('%')[0])
      let nMouseX = event.clientX
      ...
      function _setWindowTime () {
        let nStart =
          new Date(self.datetime.start).getTime() * (1 - nRatio / 100) +
          nRatio / 100 * new Date(self.datetime.end).getTime()
        self.windowTime.start = new Date(nStart).Format('yyyy-MM-dd hh:mm:ss')
        self.windowTime.end = new Date(
          nStart + self.windowTime.interval * 60 * 1000
        ).Format('yyyy-MM-dd hh:mm:ss')
    ...
    inputTimeBlur () {
      ...
      let nEnd = new Date(this.windowTime.start).getTime() + nInterval

      if (window.Rt.utils.DateDiff(nEnd, this.datetime.end) < 0) {
        // 若超出结束时间。
        this.windowTime.end = this.datetime.end
        this.windowTime.start = new Date(
          new Date(this.datetime.end).getTime() - nInterval
        ).Format('yyyy-MM-dd hh:mm:ss')
        this.windowTime.left = this.formatData(
          window.Rt.utils.DateDiff(this.datetime.start, this.windowTime.start) *
            100 /
            window.Rt.utils.DateDiff(this.datetime.start, this.datetime.end),
          2
        ) // Math.floor
      } else {

<template>
    <div class="equipment-line" :style="{width: ratio*100 + '%', left: (-windowTime.left)*ratio + '%', 'padding-top': (top-14) + 'px', 'padding-bottom': (bottom-14) + 'px'}" ref="wrap">
        ...
        <div v-for="info in dimension" :key="info" v-show="selectedDimension.indexOf(info.type)>-1" :ref="info.key" :class="info.key"></div>
    </div>
</template>
...
export default {
  ...
  created () {},
  mounted () {
    let nodes = this.$refs.wrap.parentElement.previousElementSibling.querySelectorAll(
      'label'
    )
  ...
  methods: {
              ...
              'div',
              {
                ...
                style: {
                  left:
                    ...
                    '%',
                  width:
                    window.Rt.utils.DateDiff(item.startTime, item.endTime) *
                      100 /
                      self.getDiffTime() +
                    '%'
                },
    ...
    setPanelPadding () {
      ...
      this.bottom = 14
      if (
        !this.selectedDimension.length ||
        (this.selectedDimension.length === 1 && !this.selectedDimension[0])
      ) {
        // 若无选中。
    ...
    setDimensionConfig (sType, oData) {
      ...
      let sLength = aoDimension.length
        // 事件节点。
      let oNode = oData.node
        // 节点内容高度。
      let nHeight = oNode.offsetHeight
            ...
            'div',
            {
              ...
              on: {
                ...
                mouseenter: function (event) {
                  ...
                  if (sGroup) {
                    // 获取groupId相同的节点。
                    aoNode = Array.prototype.filter.call(
                  ...
                  aoNode.forEach(o => {
                    ...
                    if (!self.visible) {
                      o.querySelector('.dimension-content').style.display =
                        'block'
                    }
                  })
                },
                  ...
                  self.$createElement(
                    ...
                    Object.keys(oList).map(p => {
                      return self.$createElement(
                        'li',
                        {},
                        oList[p] + '：' + (item[p] === undefined ? '' : item[p])
                      )
                    })
                  )
                ]
    ...
    setRepair () {
      ...
      aoSortData.map((o, index) => {
        if (o.type === 'spotCheckList') {
          this.setDimensionConfig(
            'repair',
            this._createNode(o, index, 'repair', '设备点检', {
              happenTime: '发生时间',
              personName: '操作人',
              result: '点检结果'
            })

  ...
  let oRt = {
    ...
    */
    ...
    downloadHtml: function (html2canvas, element, filename, option, callback) {
      ...
      html2canvas(element, Object.assign(option || {}, {
        onrendered: (canvas) => {
          let sImg = canvas.toDataURL('image/png')
          let oLink = document.createElement('a')
    ...
    setParam: function (sName, sValue, sUrl) {
      ...
      // 获取 URL 的参数。
      let asHref = (sUrl || window.location.href).split('?')
      let oParams = asHref[1] ? this.unparam(asHref[1]) : {}
    ...
    isDateTime: function (sTime) {
      let sReg = /^(\d+)-(\d{1,2})-(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/
      let sRs = (sTime || '').match(sReg)
    ...
    isDate: function (sDate) {
      let sReg = /^(\d+)-(\d{1,2})-(\d{1,2})$/
      let sRs = (sDate || '').match(sReg)
